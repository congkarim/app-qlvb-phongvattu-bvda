...
SMOKE_TITLE_PREFIX = "[SMOKE] Dispatch API"
SMOKE_USER_EMAIL_PREFIX = "dispatch-api-smoke-"
...
def run_smoke(*, api_base: str, keep_data: bool) -> dict[str, Any]:
    ...
    try:
        ...
        _expect_http_status(
            "GET",
            f"{api_base}/dispatches/by-document/{seed['document_id']}",
            token=user_token,
            expected_status=404,
            label="dispatch lookup before create",
        )
        ...
        created = _request_json(
            ...
            payload={
                ...
                "document_symbol": "CV-VT",
                "issued_date": "2026-06-06",
                "issuing_agency": "Phong Vat Tu Smoke",
                "recipient": "Ban Giam doc",
                "excerpt": "Cong van smoke de nghi cap vat tu",
                "status": "registered",
                "notes": "Smoke dispatch API",
            },
            expected_status=201,
        )
        created_dispatch_id = created["id"]
        _assert(created["document_id"] == seed["document_id"], "Dispatch document_id mismatch")
        _assert(created["dispatch_type"] == "incoming", "Dispatch type mismatch")

        _expect_http_status(
            "POST",
            f"{api_base}/dispatches",
            token=admin_token,
            payload={"document_id": seed["document_id"], "dispatch_type": "incoming", "status": "draft"},
            expected_status=409,
            label="duplicate active dispatch",
        )

        query = urlencode({"q": "cap vat tu", "dispatch_type": "incoming", "status": "registered", "limit": 10, "offset": 0})
        listed = _request_json("GET", f"{api_base}/dispatches?{query}", token=user_token)
        _assert(listed["total"] == 1, f"Expected one dispatch in filtered list, got {listed['total']}")
        _assert(listed["items"][0]["id"] == created_dispatch_id, "Filtered list returned unexpected dispatch")

        detail = _request_json("GET", f"{api_base}/dispatches/{created_dispatch_id}", token=user_token)
        _assert(detail["document_title"].startswith(SMOKE_TITLE_PREFIX), "Dispatch detail missing document title")
        ...
        _assert(by_document["id"] == created_dispatch_id, "Dispatch lookup by document_id mismatch")

        updated = _request_json(
            "PATCH",
            f"{api_base}/dispatches/{created_dispatch_id}",
            token=user_token,
            payload={
                "document_number": seed["document_number"],
                "issuing_agency": "Phong Vat Tu Smoke Updated",
                "recipient": "Ban Giam doc Updated",
                "excerpt": "Cong van smoke de nghi cap vat tu updated",
                "status": "completed",
                "notes": "Updated by smoke",
            },
        )
        _assert(updated["status"] == "completed", "Dispatch update did not change status")
        ...
        _expect_http_status(
            ...
            token=user_token,
            expected_status=403,
            label="user dispatch delete permission",
        )

        deleted = _request_json("DELETE", f"{api_base}/dispatches/{created_dispatch_id}", token=admin_token)
        _assert(deleted["id"] == created_dispatch_id, "Deleted response id mismatch")
        _expect_http_status(
            "GET",
            f"{api_base}/dispatches/{created_dispatch_id}",
            token=admin_token,
            expected_status=404,
            label="deleted dispatch lookup",
        )
        _expect_http_status(
            "GET",
            f"{api_base}/dispatches/by-document/{seed['document_id']}",
            token=admin_token,
            expected_status=404,
            label="deleted dispatch lookup by document",
        )
        _assert(_audit_count(db, created_dispatch_id) >= 3, "Expected dispatch create/update/delete audit logs")
...
def _seed_data(db) -> dict[str, str]:
    ...
    document = DocumentRepository(db).create_document(
        ...
        original_filename=f"dispatch-api-smoke-{suffix}.txt",
        file_path=f"/tmp/dispatch-api-smoke-{suffix}.txt",
        content_type="text/plain",
        document_type="CV",
        document_number=f"CV-SMOKE-{suffix}",
    ...
    user = UserRepository(db).create(
        email=user_email,
        full_name="Dispatch API Smoke User",
    ...
    return {
        ...
        "document_number": f"CV-API-{suffix}",
    }
...
def _audit_count(db, dispatch_id: str) -> int:
    db.expire_all()
    return len(
        list(
            db.scalars(
                select(AuditLog).where(
                    AuditLog.entity_type == "dispatch",
                    AuditLog.entity_id == dispatch_id,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Smoke test dispatch metadata API.")
    parser.add_argument("--api-base", default="http://localhost:8000/api/v1", help="API base URL.")
    parser.add_argument("--keep-data", action="store_true", help="Keep smoke document/user/dispatch for debugging.")
