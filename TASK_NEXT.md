...
Phase hiện tại: **Chưa lập phase mới.**

Mục tiêu tiếp theo: Cập nhật `ROADMAP.md` với phase ưu tiên mới (ví dụ LLM nội bộ nâng cao, inventory, workflow phê duyệt, auto-trích quan hệ OCR), rồi thay file này bằng checklist phase đó.

Điều kiện mở phase mới:
- Phase mới đã ghi trong `ROADMAP.md` với tiêu chí hoàn thành rõ ràng.
- Checklist mục tiêu đã thay thế nội dung file này.

---

## Trạng Thái

**Phase 0–15 đã hoàn thành.** Không có checklist phase đang mở.

Giới hạn / hướng mở rộng tiềm năng (xem `ROADMAP.md`):
- LLM/generator nội bộ nâng cao (RAG hiện extractive).
- Inventory/tồn kho, workflow phê duyệt nhiều bước.
- Auto-trích quan hệ document từ OCR/chunk (ngoài scope Phase 15).

Khi bắt đầu phase mới:
1. Ghi phase và tiêu chí hoàn thành trong `ROADMAP.md`.
2. Thay toàn bộ nội dung `TASK_NEXT.md` bằng checklist mục tiêu phase đó.
3. Bắt đầu từ mục tiêu 1.
